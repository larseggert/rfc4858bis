...
informative:
  IDTRACKER:
    title: The IETF Internet-Draft Tracker
    author:
    - org: ''
    date: 2002
    seriesinfo:
      Web: 'Application: https://datatracker.ietf.org/'
  PROTO:
    title: The IESG PROcess and TOols (PROTO) Team
    author:
    - org: ''
    date: 2004
    seriesinfo:
      Web: 'Page: http://psg.com/~mrw/PROTO-Team'
  GEN-ART:
    title: The General Area Review Team (GEN-ART)
    author:
    - org: ''
    date: 2005
    seriesinfo:
      Web: 'Page: http://www.alvestrand.no/ietf/gen/review-guidelines.html'
...
"PROTO process" or "PROTO" (because it was created by the "PROcess and TOols"
or PROTO {{PROTO}} team), is a set of methodologies designed
...
# Process Description {#sect-3}

The document shepherding process consists of the following tasks:

- Providing the Document Shepherd Write-Up accompanying a document
  that is forwarded to the IESG when publication is requested, as
  described in {{sect-3.1}}.

- During AD Evaluation of the document by the Responsible Area
  Director, managing the discussion between the editors, the working
  group, and the Responsible Area Director, as described in {{sect-3.2}}.

- During an IETF Last Call, if performed for the shepherded
...
- During IESG Evaluation, following up on all IESG feedback
  ...
  document, as described in {{sect-3.3}}.

- Following up on IANA and RFC Editor requests as described in {{sect-4}} and {{sect-5}}.
...
{{sect-6}} discusses other instances in which the document shepherding
process does not apply.

## Document Shepherd Write-Up {#sect-3.1}
...
The second part of the task is to prepare the "Document Announcement Write-Up"
that is input both to the ballot for the IESG telechat and
...
: Has the Document Shepherd personally verified that the
  document satisfies all ID nits?  (See [](http://www.ietf.org/ID-Checklist.html)
...
the Document Shepherd MUST also be added to the "State or Version Change
...
## Document Shepherding during AD Evaluation {#sect-3.2}
...
{: hangIndent='7'}
(2.a)
: The Responsible Area Director reads, evaluates, and comments
  ...
  document shepherding process continues as described in {{sect-3.3}}.

(2.b)
: If the Responsible Area Director has identified issues with a
...
(2.c)
: The Document Shepherd reads the AD Evaluation comments, making
...
(2.d)
: The Document Shepherd sends the AD Evaluation comments to the
...
(2.f)
: In the event that the editors or working group disagrees with
...
## Document Shepherding during IESG Evaluation {#sect-3.3}
...
: Immediately following the conference call, when the document
  ...
  states requiring Document Shepherd action, e.g., "IESG Evaluation: Revised
  ID Needed" or "IESG Evaluation: AD Followup", the Document Shepherd will
  receive email.  A state
...
: The Document Shepherd then coordinates the resolution of
  ...
  of the process described in {{sect-3.2}}.
...
: The Document Shepherd then communicates the DISCUSS and
  ...
  accumulated during IESG processing, such as "Notes to the RFC Editor".  If
  any changes will be substantive, the Document
...
: After the editors resolve the DISCUSS and COMMENT items, the
  ...
  document, which will be a revised document, a set of "Notes to the RFC Editor",
  or both, using his or her technical expertise
...
: Each AD who had raised a DISCUSS checks whether the
  ...
  possibilities exist:
   ...
   (b)
   : If no progress can be made on the resolution of the
...
# Shepherding the Document's IANA Actions {#sect-4}
...
At the time of this publication, RFC 2434 is under revision {{?RFC2434bis}},
and the updates are and will be of value to the
...
# Document Shepherding after IESG Approval {#sect-5}

After the IESG Evaluation and resolution described in {{sect-3.3}},
...
# When Not to Use the Document Shepherding Process {#sect-6}

As mentioned in {{sect-3}}, the Document Shepherd SHOULD NOT be an
...
1. Cases where the Responsible Area Director expects communication
  difficulties with the Document Shepherd (either due to
  experience, strong views stated by the Document Shepherd, or
  other issues).

1. Cases where the working group itself is either very old, losing
...
# Security Considerations {#sect-7}
...
# Example Document Announcement Write-Ups {#sect-a}
...
## Example Document Announcement Write-Up for draft-ietf-avt-rtp-midi-format {#sect-a.1}
...
## Example Document Announcement Write-Up for draft-ietf-imss-ip-over-fibre-channel {#sect-a.2}
